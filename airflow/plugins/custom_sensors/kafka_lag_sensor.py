"""
Custom Airflow Sensor: Kafka Consumer Group Lag Sensor

Waits for a Kafka consumer group's total lag to drop below a configured
threshold. Useful for gating tasks that depend on near-real-time data
being fully consumed (e.g., before running Trino queries over Iceberg
...
class KafkaLagSensor(BaseSensorOperator):
    """
    Sensor that waits for Kafka consumer group lag to drop below a threshold.
    ...
    def _get_consumer_lag(self) -> dict:
        """
        Query Kafka for consumer group lag using confluent_kafka AdminClient.
        ...
        try:
            ...
            logger.error("Error fetching Kafka consumer lag: %s", str(e))
            result["error"] = str(e)
    ...
    def poke(self, context: Context) -> bool:
        ...
        """
        logger.info(
            "Checking Kafka lag for group '%s' (max_lag=%d, topics=%s)",
            self.consumer_group,
            self.max_lag,
            self.topics or "all",
        )
